...
public class WorldGuard6Manager implements PrivatePlugin{
	...
	private boolean outsidePrivate(Player player, Location loc) {
		ApplicableRegionSet regions = wg.getRegionManager(player.getWorld()).getApplicableRegions(loc);
		if (regions.size() < 1){
            return true;
        }else{
        	LocalPlayer lp = wg.wrapPlayer(player);
	...
	public List<String> getRegions(Location location) {
		...
		ApplicableRegionSet regions = wg.getRegionManager(location.getWorld()).getApplicableRegions(location);
		for(ProtectedRegion rg : regions){
			l.add(rg.getId());
		}
		return l;
