package ru.mor.iv.mineblocker.full.delete.packet.v1_5_R3;
...
public class PacketUtils implements PacketUtilsInterface{
    ...
    public Inventory fromString(String data) throws IOException {
        ...
        NBTTagList itemList = NBTCompressedStreamTools.a(inputStream).getList("Inventory"); ////// getList("Inventory", 10);
        Inventory inventory = new CraftInventoryCustom(null, itemList.size());

        for (int i = 0; i < itemList.size(); i++) {
        	NBTTagCompound inputObject = get(itemList, i);
	...
	public NBTTagCompound get(NBTTagList itemList, int paramInt)
	  {
	    if ((paramInt < 0) || (paramInt >= itemList.size())) {
	      return new NBTTagCompound();
	    }
	    NBTBase localNBTBase = (NBTBase)itemList.get(paramInt);
